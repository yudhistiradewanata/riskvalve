...
) : Controller
{
    ...
    )
    {
        ...
        if (!string.IsNullOrEmpty(searchValue))
        {
            assetList = assetList
                .Where(x =>
                       (x.TagNo != null && x.TagNo.Contains(searchValue, StringComparison.CurrentCultureIgnoreCase))
                    || (x.AssetName != null && x.AssetName.Contains(searchValue, StringComparison.CurrentCultureIgnoreCase))
                    || (x.BusinessArea != null && x.BusinessArea.Contains(searchValue, StringComparison.CurrentCultureIgnoreCase))
                    || (x.Platform != null && x.Platform.Contains(searchValue, StringComparison.CurrentCultureIgnoreCase))
                    || (x.ValveType != null && x.ValveType.Contains(searchValue, StringComparison.CurrentCultureIgnoreCase))
                    || (x.Size != null && x.Size.Contains(searchValue, StringComparison.CurrentCultureIgnoreCase))
                    || (x.ClassRating != null && x.ClassRating.Contains(searchValue, StringComparison.CurrentCultureIgnoreCase))
                    || (x.PIDNo != null && x.PIDNo.Contains(searchValue, StringComparison.CurrentCultureIgnoreCase))
        ...
        if (!string.IsNullOrEmpty(sortColumn) && !string.IsNullOrEmpty(sortColumnDirection))
        {
            ...
            {
                assetList =
                    sortColumnDirection.Equals("asc", StringComparison.CurrentCultureIgnoreCase)
    ...
    public IActionResult UpdateAsset()
    {
        ...
        try{
            ...
            int platformId = string.IsNullOrEmpty(platformIdString) ? 0 : SharedEnvironment.StringToInt(platformIdString);
            string? valveTypeIdString = Request.Form["ValveTypeID"];
            int valveTypeId = string.IsNullOrEmpty(valveTypeIdString) ? 0 : SharedEnvironment.StringToInt(valveTypeIdString);
            string? manualOverrideIdString = Request.Form["ManualOverrideID"];
            int manualOverrideId = string.IsNullOrEmpty(manualOverrideIdString) ? 0 : SharedEnvironment.StringToInt(manualOverrideIdString);
            string? fluidPhaseIdString = Request.Form["FluidPhaseID"];
            int fluidPhaseId = string.IsNullOrEmpty(fluidPhaseIdString) ? 0 : SharedEnvironment.StringToInt(fluidPhaseIdString);
            string? toxicOrFlamableFluidIdString = Request.Form["ToxicOrFlamableFluidID"];
            int toxicOrFlamableFluidId = string.IsNullOrEmpty(toxicOrFlamableFluidIdString) ? 0 : SharedEnvironment.StringToInt(toxicOrFlamableFluidIdString);
            AssetClass asset = new()
            {
                Id = id,
    ...
    public IActionResult GetAssetSidebar(int PlatformID)
    {
        ...
        Console.WriteLine("=== END DEBUG ===");
        var selectedAssetList = assetList.Select(
            asset => new{
                asset.Id,
                asset.TagNo,
                asset.BusinessArea,
    ...
    {
        List<AssetData> assetDatas = _assetService.GetAssetList()
        .Select(
            asset => new AssetData
            {
                Id = asset.Id,
        ...
        .Where(
            a => a.TagNo != null && a.TagNo.Contains(search, StringComparison.CurrentCultureIgnoreCase)
        ).ToList();
