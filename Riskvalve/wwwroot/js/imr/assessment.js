...
$(document).ready(function () {
	...
	if (idParam) {
		$(
			".btn-add-new-inspection[attr-inspection-id=" + idParam + "]"
		).trigger("click");
	} else if (assetIdParam) {
		$(".btn-add-new-inspection[attr-id=" + assetIdParam + "]")
			.first()
			.trigger("click");
	}
});
...
function submitForm(el) {
	...
	const selectedInspectionCount = form.find(
		"input[name=selectedInspectionId]"
	).length;
	var alertmessage = "";
			...
			$.ajax({
				...
				success: function (apiresult) {
					if (apiresult.isSuccess) {
						...
						$(
							".btn-add-new-inspection[attr-inspection-id=" +
								res.id +
								"]"
						).html(res.assessmentDate);
						fetchHistoryData({
							id: res.id,
							counter: ctr,
						});
			...
			$.ajax({
				...
				success: function (apiresult) {
					if (apiresult.isSuccess) {
						...
						// add menu to tree
						$('ul[equipment-id="' + equipmentId + '"] li').eq(0)
							.after(`
					<li class="treeview">
					...
					`);
						$(".nav-tab-menus")
							.find("a[tab-num=" + ctr + "] div[tab-title]")
							.text(res.asset.tagNo + " " + res.assessmentDate);
		...
		$.ajax({
			...
			success: function (apiresponse) {
				if (apiresponse.isSuccess) {
					...
					$(".btn-add-new-inspection[attr-inspection-id=" + id + "]")
						.closest("li")
						.remove();
					// Handle success response
				} else {
	...
	$.ajax({
		...
		success: function (apiresult) {
			if (apiresult.isSuccess) {
				...
				const editable = assessmentCalculatedField.some(
				...
				if (!editable) {
					...
					$("#inspection" + counter)
						.find(".print-button")
						.attr("href", urlassessmentprint + "/" + id);
				...
				$("#field-inspection-inspection-method" + counter).val(
					data.inspectionMethodID
				);
				$(
					"#field-inspection-current-condition-limit-state-a" +
						counter
				).val(data.currentConditionLeakeageToAtmosphereID);
				$(
					"#field-inspection-current-condition-limit-state-b" +
						counter
				).val(data.currentConditionFailureOfFunctionID);
				$(
					"#field-inspection-current-condition-limit-state-c" +
						counter
				).val(data.currentConditionPassingAcrossValveID);
				$("#field-inspection-effectiveness" + counter).val(
					data.inspectionEffectivenessID
				);
				$("#field-inspection-function-condition" + counter).val(
					data.functionCondition
				);
				$("#field-inspection-description" + counter).val(
					data.inspectionDescription
				);
				$("#field-inspection-test-pressure-if-any" + counter).val(
					data.testPressureIfAny
				);
				$("#maintenanceDataTemp" + counter).val(
					JSON.stringify(data.maintenanceHistory)
				);
				$("#inspectionDataTemp" + counter).val(
					JSON.stringify(data.inspectionHistory)
				);
				...
				$("#time-to-limit-a" + counter).val(
					data.timeToLimitStateLeakageToAtmosphere
				);
				$("#time-to-limit-b" + counter).val(
					data.timeToLimitStateFailureOfFunction
				);
				$("#time-to-limit-c" + counter).val(
					data.timeToLimitStatePassingAccrosValve
				);
				$("#inspection-effectiveness" + counter).val(
					data.inspectionEffectivenessID
				);
				$("#impact-of-internal-fluid" + counter).val(
					data.impactOfInternalFluidImpuritiesID
				);
				$("#impact-of-operating-envelopes" + counter).val(
					data.impactOfOperatingEnvelopesID
				);
				$("#used-within-oem-specification" + counter).val(
					data.usedWithinOEMSpecificationID
				);
				$("#repaired" + counter).val(data.repairedID);
				$("#product-loss-definition" + counter).val(
					data.productLossDefinition
				);
				...
				if (data.recommendationActionID != null) {
					$("#recommended-action" + counter).val(
						data.recommendationActionID
					);
				}
				$("#detailed-recommendation" + counter).val(
					data.detailedRecommendation
				);

				//LF1 & LF2
				console.log("data", data);
				$("#lastInspectionDate" + counter).val(data.inspectionDate);
				$("#leakage-to-atmosphere" + counter).val(
					data.leakageToAtmosphereID
				);
				$("#failure-of-function" + counter).val(
					data.failureOfFunctionID
				);
				$("#passing-accross-valve" + counter).val(
					data.passingAccrosValveID
				);
				...
				$("#inspection" + counter)
					.find("form")
					.addClass("history-form");
				$("#inspection" + counter)
					.find(".calculate-assessment")
					.first()
					.trigger("change");
...
function fillAssetInfo({ counter, data }) {
	...
	$("#field-assessment-operatingtemperature" + counter).val(
		data.operatingTemperature
	);
	$("#field-assessment-operatingpressure" + counter).val(
		data.operatingPressure
	);
	$("#field-assessment-toxicorflamableliquid" + counter).val(
	...
	$("#field-assessment-actuatorserialno" + counter).val(
		data.actuatorSerialNo
	);
	$("#field-assessment-actuatortypemodel" + counter).val(
...
function calculateXPos(value) {
	...
	console.log("X-Curernt Value: ", value);
	console.log("X-Curernt Position: ", xpos);
	return xpos;
}
...
function calculateAssessment(ctr = -1) {
	...
	var lastInspectionDate = $('.tab-pane[tab-num="' + ctr + '"]')
		.find("#lastInspectionDate")
		.val();
	...
	var calculated_lf2LeakageToAtmosphereTP1 = decide_pos_ttl(
		timeToLimitAint,
		timePeriod
	);
	var calculated_lf2LeakageToAtmosphereTP2 = decide_pos_ttl(
		timeToLimitAint,
		timePeriod * 2
	);
	var calculated_lf2LeakageToAtmosphereTP3 = decide_pos_ttl(
		timeToLimitAint,
		timePeriod * 3
	);
	var calculated_lf2FailureOfFunctionTP1 = decide_pos_ttl(
		timeToLimitBint,
		timePeriod
	);
	var calculated_lf2FailureOfFunctionTP2 = decide_pos_ttl(
		timeToLimitBint,
		timePeriod * 2
	);
	var calculated_lf2FailureOfFunctionTP3 = decide_pos_ttl(
		timeToLimitBint,
		timePeriod * 3
	);
	var calculated_lf2PassingAccrossValveTP1 = decide_pos_ttl(
		timeToLimitCint,
		timePeriod
	);
	var calculated_lf2PassingAccrossValveTP2 = decide_pos_ttl(
		timeToLimitCint,
		timePeriod * 2
	);
	var calculated_lf2PassingAccrossValveTP3 = decide_pos_ttl(
		timeToLimitCint,
		timePeriod * 3
	);
	...
	//LF1
	var lf1LeakageToAtmosphere = $("#leakage-to-atmosphere" + ctr)
		.find("option:selected")
		.attr("attr-value")
		.replace(",", ".");
	var lf1LeakageToAtmosphereWeighting = $("#leakage-to-atmosphere" + ctr)
		.find("option:selected")
		.attr("attr-weighting")
		.replace(",", ".");
	var lf1FailureOfFunction = $("#failure-of-function" + ctr)
		.find("option:selected")
		.attr("attr-value")
		.replace(",", ".");
	var lf1FailureOfFunctionWeighting = $("#failure-of-function" + ctr)
		.find("option:selected")
		.attr("attr-weighting")
		.replace(",", ".");
	var lf1PassingAccrossValve = $("#passing-accross-valve" + ctr)
		.find("option:selected")
		.attr("attr-value")
		.replace(",", ".");
	var lf1PassingAccrossValveWeighting = $("#passing-accross-valve" + ctr)
		.find("option:selected")
		.attr("attr-weighting")
		.replace(",", ".");
	//LF2
	var lf2LeakageToAtmosphereTP1 = $("#leakage-tp1" + ctr)
		.find("option:selected")
		.attr("attr-value")
		.replace(",", ".");
	var lf2LeakageToAtmosphereTP1Weighting = $("#leakage-tp1" + ctr)
		.find("option:selected")
		.attr("attr-weighting")
		.replace(",", ".");
	var lf2LeakageToAtmosphereTP1Value = $("#leakage-tp1" + ctr)
		.find("option:selected")
		.val();
	var lf2LeakageToAtmosphereTP2 = $("#leakage-tp2" + ctr)
		.find("option:selected")
		.attr("attr-value")
		.replace(",", ".");
	var lf2LeakageToAtmosphereTP2Weighting = $("#leakage-tp2" + ctr)
		.find("option:selected")
		.attr("attr-weighting")
		.replace(",", ".");
	var lf2LeakageToAtmosphereTP2Value = $("#leakage-tp2" + ctr)
		.find("option:selected")
		.val();
	var lf2LeakageToAtmosphereTP3 = $("#leakage-tp3" + ctr)
		.find("option:selected")
		.attr("attr-value")
		.replace(",", ".");
	var lf2LeakageToAtmosphereTP3Weighting = $("#leakage-tp3" + ctr)
		.find("option:selected")
		.attr("attr-weighting")
		.replace(",", ".");
	var lf2LeakageToAtmosphereTP3Value = $("#leakage-tp3" + ctr)
		.find("option:selected")
		.val();
	var lf2FailureOfFunctionTP1 = $("#failure-tp1" + ctr)
		.find("option:selected")
		.attr("attr-value")
		.replace(",", ".");
	var lf2FailureOfFunctionTP1Weighting = $("#failure-tp1" + ctr)
		.find("option:selected")
		.attr("attr-weighting")
		.replace(",", ".");
	var lf2FailureOfFunctionTP1Value = $("#failure-tp1" + ctr)
		.find("option:selected")
		.val();
	var lf2FailureOfFunctionTP2 = $("#failure-tp2" + ctr)
		.find("option:selected")
		.attr("attr-value")
		.replace(",", ".");
	var lf2FailureOfFunctionTP2Weighting = $("#failure-tp2" + ctr)
		.find("option:selected")
		.attr("attr-weighting")
		.replace(",", ".");
	var lf2FailureOfFunctionTP2Value = $("#failure-tp2" + ctr)
		.find("option:selected")
		.val();
	var lf2FailureOfFunctionTP3 = $("#failure-tp3" + ctr)
		.find("option:selected")
		.attr("attr-value")
		.replace(",", ".");
	var lf2FailureOfFunctionTP3Weighting = $("#failure-tp3" + ctr)
		.find("option:selected")
		.attr("attr-weighting")
		.replace(",", ".");
	var lf2FailureOfFunctionTP3Value = $("#failure-tp3" + ctr)
		.find("option:selected")
		.val();
	var lf2PassingAccrossValveTP1 = $("#passing-tp1" + ctr)
		.find("option:selected")
		.attr("attr-value")
		.replace(",", ".");
	var lf2PassingAccrossValveTP1Weighting = $("#passing-tp1" + ctr)
		.find("option:selected")
		.attr("attr-weighting")
		.replace(",", ".");
	var lf2PassingAccrossValveTP1Value = $("#passing-tp1" + ctr)
		.find("option:selected")
		.val();
	var lf2PassingAccrossValveTP2 = $("#passing-tp2" + ctr)
		.find("option:selected")
		.attr("attr-value")
		.replace(",", ".");
	var lf2PassingAccrossValveTP2Weighting = $("#passing-tp2" + ctr)
		.find("option:selected")
		.attr("attr-weighting")
		.replace(",", ".");
	var lf2PassingAccrossValveTP2Value = $("#passing-tp2" + ctr)
		.find("option:selected")
		.val();
	var lf2PassingAccrossValveTP3 = $("#passing-tp3" + ctr)
		.find("option:selected")
		.attr("attr-value")
		.replace(",", ".");
	var lf2PassingAccrossValveTP3Weighting = $("#passing-tp3" + ctr)
		.find("option:selected")
		.attr("attr-weighting")
		.replace(",", ".");
	var lf2PassingAccrossValveTP3Value = $("#passing-tp3" + ctr)
		.find("option:selected")
		.val();
	//LF3
	if (
		$("#inspection-effectiveness" + ctr).find("option:selected").length ==
			0 ||
		$("#inspection-effectiveness" + ctr)
			.find("option:selected")
			.val() == ""
	) {
		return;
	}
	var lf3InspectionEffectiveness = $("#inspection-effectiveness" + ctr)
		.find("option:selected")
		.attr("attr-value")
		.replace(",", ".");
	var lf3InspectionEffectivenessWeighting = $(
		"#inspection-effectiveness" + ctr
	)
		.find("option:selected")
		.attr("attr-weighting")
	...
	//LF4
	if (
		$("#impact-of-internal-fluid" + ctr).find("option:selected").length ==
			0 ||
		$("#impact-of-internal-fluid" + ctr)
			.find("option:selected")
			.val() == ""
	) {
		return;
	}
	var lf4ImpactOfInternalFluid = $("#impact-of-internal-fluid" + ctr)
		.find("option:selected")
		.attr("attr-value")
		.replace(",", ".");
	var lf4ImpactOfInternalFluidWeighting = $("#impact-of-internal-fluid" + ctr)
		.find("option:selected")
		.attr("attr-weighting")
		.replace(",", ".");
	//LF5
	if (
		$("#impact-of-operating-envelopes" + ctr).find("option:selected")
			.length == 0 ||
		$("#impact-of-operating-envelopes" + ctr)
			.find("option:selected")
			.val() == ""
	) {
		return;
	}
	var lf5ImpactOfOperatingEnvelopes = $(
		"#impact-of-operating-envelopes" + ctr
	)
		.find("option:selected")
		.attr("attr-value")
		.replace(",", ".");
	var lf5ImpactOfOperatingEnvelopesWeighting = $(
		"#impact-of-operating-envelopes" + ctr
	)
		.find("option:selected")
		.attr("attr-weighting")
	...
	//LF6
	if (
		$("#used-within-oem-specification" + ctr).find("option:selected")
			.length == 0 ||
		$("#used-within-oem-specification" + ctr)
			.find("option:selected")
			.val() == ""
	) {
		return;
	}
	var lf6UsedWithinOEMSpecification = $(
		"#used-within-oem-specification" + ctr
	)
		.find("option:selected")
		.attr("attr-value")
		.replace(",", ".");
	var lf6UsedWithinOEMSpecificationWeighting = $(
		"#used-within-oem-specification" + ctr
	)
		.find("option:selected")
		.attr("attr-weighting")
	...
	//LF7
	if (
		$("#repaired" + ctr).find("option:selected").length == 0 ||
		$("#repaired" + ctr)
			.find("option:selected")
			.val() == ""
	) {
	...
	var lf7Repaired = $("#repaired" + ctr)
		.find("option:selected")
		.attr("attr-value")
		.replace(",", ".");
	var lf7RepairedWeighting = $("#repaired" + ctr)
		.find("option:selected")
		.attr("attr-weighting")
		.replace(",", ".");
	//CALCULATE
	// TP1A
	var lofLeakageToAtmosphereA =
		lf2LeakageToAtmosphereTP1 *
		lf2LeakageToAtmosphereTP1Weighting *
		(lf1LeakageToAtmosphere * lf1LeakageToAtmosphereWeighting +
			...
			lf5ImpactOfOperatingEnvelopes *
				lf5ImpactOfOperatingEnvelopesWeighting +
			lf6UsedWithinOEMSpecification *
				lf6UsedWithinOEMSpecificationWeighting +
	...
	console.log(
		"LF2",
		lf2LeakageToAtmosphereTP1 * lf2LeakageToAtmosphereTP1Weighting
	);
	console.log("lf1LeakageToAtmosphere", lf1LeakageToAtmosphere);
	console.log(
		"lf1LeakageToAtmosphereWeighting",
		lf1LeakageToAtmosphereWeighting
	);
	console.log(
		"LF1",
	...
	console.log(
		"lf3InspectionEffectivenessWeighting",
		lf3InspectionEffectivenessWeighting
	);
	console.log(
		"LF3",
	...
	console.log(
		"lf4ImpactOfInternalFluidWeighting",
		lf4ImpactOfInternalFluidWeighting
	);
	console.log(
		"LF4",
	...
	console.log(
		"calculator lf2 * (lf1 + lf3 + lf4 + lf5 + lf6 + lf7)",
		lofLeakageToAtmosphereA
	);
	console.log("=== END TP1A CALCULARTOR ===");
	var lofLeakageToAtmosphereAPos = calculateXPos(lofLeakageToAtmosphereA);
	// TP1B
	var lofLeakageToAtmosphereB =
		lf2LeakageToAtmosphereTP2 *
		lf2LeakageToAtmosphereTP2Weighting *
		(lf1LeakageToAtmosphere * lf1LeakageToAtmosphereWeighting +
			lf3InspectionEffectiveness * lf3InspectionEffectivenessWeighting +
			lf4ImpactOfInternalFluid * lf4ImpactOfInternalFluidWeighting +
			lf5ImpactOfOperatingEnvelopes *
	...
	// TP1C
	var lofLeakageToAtmosphereC =
		lf2LeakageToAtmosphereTP3 *
		lf2LeakageToAtmosphereTP3Weighting *
		(lf1LeakageToAtmosphere * lf1LeakageToAtmosphereWeighting +
			lf3InspectionEffectiveness * lf3InspectionEffectivenessWeighting +
			lf4ImpactOfInternalFluid * lf4ImpactOfInternalFluidWeighting +
			lf5ImpactOfOperatingEnvelopes *
	...
	// TP2A
	var lofFailureOfFunctionA =
		lf2FailureOfFunctionTP1 *
		lf2FailureOfFunctionTP1Weighting *
	...
	// TP2B
	var lofFailureOfFunctionB =
		lf2FailureOfFunctionTP2 *
		lf2FailureOfFunctionTP2Weighting *
		(lf1FailureOfFunction * lf1FailureOfFunctionWeighting +
			lf3InspectionEffectiveness * lf3InspectionEffectivenessWeighting +
			lf4ImpactOfInternalFluid * lf4ImpactOfInternalFluidWeighting +
			lf5ImpactOfOperatingEnvelopes *
	...
	// TP2C
	var lofFailureOfFunctionC =
		lf2FailureOfFunctionTP3 *
		lf2FailureOfFunctionTP3Weighting *
		(lf1FailureOfFunction * lf1FailureOfFunctionWeighting +
			lf3InspectionEffectiveness * lf3InspectionEffectivenessWeighting +
			lf4ImpactOfInternalFluid * lf4ImpactOfInternalFluidWeighting +
			lf5ImpactOfOperatingEnvelopes *
	...
	// TP3A
	var lofPassingAccrossValveA =
		lf2PassingAccrossValveTP1 *
		lf2PassingAccrossValveTP1Weighting *
	...
	// TP3B
	var lofPassingAccrossValveB =
		lf2PassingAccrossValveTP2 *
		lf2PassingAccrossValveTP2Weighting *
		(lf1PassingAccrossValve * lf1PassingAccrossValveWeighting +
			lf3InspectionEffectiveness * lf3InspectionEffectivenessWeighting +
			lf4ImpactOfInternalFluid * lf4ImpactOfInternalFluidWeighting +
			lf5ImpactOfOperatingEnvelopes *
	...
	// TP3C
	var lofPassingAccrossValveC =
		lf2PassingAccrossValveTP3 *
		lf2PassingAccrossValveTP3Weighting *
		(lf1PassingAccrossValve * lf1PassingAccrossValveWeighting +
			lf3InspectionEffectiveness * lf3InspectionEffectivenessWeighting +
			lf4ImpactOfInternalFluid * lf4ImpactOfInternalFluidWeighting +
			lf5ImpactOfOperatingEnvelopes *
	...
	if (
		hsseSelectReal.find("option:selected").length == 0 ||
		hsseSelectReal.find("option:selected").val() == ""
	) {
		return;
	}
	...
	console.log("XX-cof1", cof1);
	console.log("XX-cof2", cof2);
	var cof = String.fromCharCode(
		Math.max(cof1.charCodeAt(0), cof2.charCodeAt(0))
	);
	var cofval = Math.max(
		hsseSelectReal.find("option:selected").attr("attr-score"),
		hsseSelect.find("option:selected").attr("attr-score")
	);
	var cof_rac = [];
	cof_rac["A"] = 3769;
	cof_rac["B"] = 891;
	...
	var tpos1a_lof = Math.ceil(lofLeakageToAtmosphereAPos / 20 + 0);
	console.log("X=tPos1A", lofLeakageToAtmosphereAPos);
	if (tpos1a_lof > 5) {
		tpos1a_lof = 5;
	}
	var tpos1a = tpos1a_lof + cof;
	$("#tP1A" + ctr).val(tpos1a);
	var tpos1b_lof = Math.ceil(lofLeakageToAtmosphereBPos / 20 + 0);
	console.log("X=tPos1B", lofLeakageToAtmosphereBPos);
	if (tpos1b_lof > 5) {
		tpos1b_lof = 5;
	}
	var tpos1b = tpos1b_lof + cof;
	$("#tP1B" + ctr).val(tpos1b);
	var tpos1c_lof = Math.ceil(lofLeakageToAtmosphereCPos / 20 + 0);
	console.log("X=tPos1C", lofLeakageToAtmosphereCPos);
	if (tpos1c_lof > 5) {
		tpos1c_lof = 5;
	}
	var tpos1c = tpos1c_lof + cof;
	$("#tP1C" + ctr).val(tpos1c);
	var tpos2a_lof = Math.ceil(lofFailureOfFunctionAPos / 20 + 0);
	console.log("X=tPos2A", lofFailureOfFunctionAPos);
	if (tpos2a_lof > 5) {
		tpos2a_lof = 5;
	}
	var tpos2a = tpos2a_lof + cof;
	$("#tP2A" + ctr).val(tpos2a);
	var tpos2b_lof = Math.ceil(lofFailureOfFunctionBPos / 20 + 0);
	console.log("X=tPos2B", lofFailureOfFunctionBPos);
	if (tpos2b_lof > 5) {
		tpos2b_lof = 5;
	}
	var tpos2b = tpos2b_lof + cof;
	$("#tP2B" + ctr).val(tpos2b);
	var tpos2c_lof = Math.ceil(lofFailureOfFunctionCPos / 20 + 0);
	console.log("X=tPos2C", lofFailureOfFunctionCPos);
	if (tpos2c_lof > 5) {
		tpos2c_lof = 5;
	}
	var tpos2c = tpos2c_lof + cof;
	$("#tP2C" + ctr).val(tpos2c);
	var tpos3a_lof = Math.ceil(lofPassingAccrossValveAPos / 20 + 0);
	console.log("X=tPos3A", lofPassingAccrossValveAPos);
	if (tpos3a_lof > 5) {
		tpos3a_lof = 5;
	}
	var tpos3a = tpos3a_lof + cof;
	$("#tP3A" + ctr).val(tpos3a);
	var tpos3b_lof = Math.ceil(lofPassingAccrossValveBPos / 20 + 0);
	console.log("X=tPos3B", lofPassingAccrossValveBPos);
	if (tpos3b_lof > 5) {
		tpos3b_lof = 5;
	}
	var tpos3b = tpos3b_lof + cof;
	$("#tP3B" + ctr).val(tpos3b);
	var tpos3c_lof = Math.ceil(lofPassingAccrossValveCPos / 20 + 0);
	console.log("X=tPos3C", lofPassingAccrossValveCPos);
	if (tpos3c_lof > 5) {
		tpos3c_lof = 5;
	}
	...
	for (const [key, item] of Object.entries(map_result)) {
		...
		const dotLabel = $(".tab-pane[tab-num=" + ctr + "]").find(
			"[" + key + "-label]"
		);
		dotLabel.text(item.label);

		const dotColorLabel = $(".tab-pane[tab-num=" + ctr + "]").find(
			"[" + key + "-colorlabel]"
		);
	...
	var currentIntegrityStatus = $(".tab-pane[tab-num=" + ctr + "]").find(
		"#integrity-status" + ctr
	);
	if (currentIntegrityStatus.attr("x-fromdb") != "true") {
		$(".tab-pane[tab-num=" + ctr + "]")
			.find("#integrity-status" + ctr)
			.val(getIntegrityStatus(tp1color));
	}
	...
	var currentTTA = $(".tab-pane[tab-num=" + ctr + "]").find(
		"#time-to-action" + ctr
	);
	if (currentTTA.attr("x-fromdb") != "true") {
		$(".tab-pane[tab-num=" + ctr + "]")
			.find("#time-to-action" + ctr)
			.val(map_ttl["TPRISK-TTA"]);
	}
}

function decideTTARAC(
	loftp1,
	loftp2,
	loftp3,
	timelimittp1,
	...
	if (loftp1 > rac) {
		...
		if (loftp3 > rac && loftp2 <= rac) {
			...
			/* 54-36 * 49-29 /290-29 + */
			calculate =
				(((timelimittp3 - timelimittp2) / 12) * (rac - loftp2)) /
					(loftp3 - loftp2) +
				timelimittp2 / 12;
			...
			console.log("loftp2: " + loftp2);
			calculate =
				(((timelimittp2 - timelimittp1) / 12) * (rac - loftp1)) /
					(loftp2 - loftp1) +
				timelimittp1 / 12;
			calculate = Math.floor(calculate * 12);
...
function addTab() {
	...
	if (tabInspectionId > 0) {
		if (
			$('.tab-pane[tab-inspection-id="' + tabInspectionId + '"]').length >
			0
		) {
	...
	tabheadercontainer.append(
		'<li id="inspectionheader' +
			ctr +
			'"><a href="#inspection' +
			ctr +
			'" data-bs-toggle="tab" class="nav-link" data-bs-target="#inspection' +
			ctr +
			'" type="button" role="tab" tab-num="' +
			ctr +
			'"><div tab-title>' +
			name +
			'</div><button type="button" class="btn-close-inspection"><i class="fa-solid fa-xmark"></i></button></a></li>'
	);
	tabcontentcontainer.append(
		'<div class="tab-pane" tab-inspection-id="' +
			tabInspectionId +
			'" equipment-id="' +
			equipmentId +
			'" id="inspection' +
				...
				if (apiresult.isSuccess == true) {
					...
					data.forEach((item, idx) => {
						html += `
							...
							<input id="addInspectionRow${item.id}" ${
							selectedId.includes(String(item.id))
								? "checked"
								: ""
						} type="checkbox" name="selectedId" value="${item.id}">
									<!-- <label for="addInspectionRow${
										item.id
									}" class="btn btn-light btn-sm">Select</label> -->
							</div>
							</td>
							</tr>
							`;
					});
					$(modalId + " input[name=tempData]").val(
						JSON.stringify(data)
					);
...
const changeContentCard = (key, el) => {
	...
	$(el)
		.closest(".assessment-tab-content-submenu")
		.find(".submenu-item")
		.removeClass("active");
	$(el).addClass("active");
...
function confirmInspection(el) {
	...
	$(".tab-pane[tab-num=" + tabNum + "] .inspection-data-temp").val(
...
function fillInspectionTable(tabNum, data) {
	...
	data.forEach((item, idx) => {
		html += `
			...
			<a class="btn btn-light btn-sm" onclick="detailInspection(${
				item.id
			})">Detail</a>
			<a class="btn btn-light btn-sm only-add" onclick="deleteInspection(this, ${
				item.id
			})">Delete</a>
			</td>
			</tr>
	...
	$("#leakage-to-atmosphere" + tabNum).val(
		latestData.currentConditionLeakeageToAtmosphereID
	);
	$("#failure-of-function" + tabNum).val(
		latestData.currentConditionFailureOfFunctionID
	);
	$("#passing-accross-valve" + tabNum).val(
		latestData.currentConditionPassingAcrossValveID
	);
	$("#inspection-effectiveness" + tabNum).val(
		latestData.inspectionEffectivenessID
	);
	initDatatable({
		scrollable: false,
...
function confirmMaintenance(el) {
	...
	$(".tab-pane[tab-num=" + tabNum + "] .maintenance-data-temp").val(
		JSON.stringify(selectedRows)
	);
...
function fillMaintenanceTable(tabNum, data) {
	const tableSelector =
		".tab-pane[tab-num=" + tabNum + "] .maintenance-table";
	$(tableSelector).DataTable().destroy();
	const table = $(tableSelector + " tbody");
	table.html("");
	...
	data.forEach((item, idx) => {
		html += `
			...
			<a class="btn btn-light btn-sm" onclick="detailMaintenance(${
				item.id
			})">Detail</a>
			<a class="btn btn-light btn-sm only-add" onclick="deleteMaintenance(this, ${
				item.id
			})">Delete</a>
			</td>
			</tr>
...
function detailInspection(id) {
	...
	$("#detailInspectionModal .preview-image-gallery")
		.find("input[type=file], .single-image")
		.remove();
...
function detailMaintenance(id) {
	...
	$("#detailMaintenanceModal .preview-image-gallery")
		.find("input[type=file], .single-image")
		.remove();
	...
	$.ajax({
		...
		success: function (apiresult) {
			if (apiresult.isSuccess) {
				...
				$(modalSelector + " #field-inspection-assetid").val(
					dataAsset.id
				);
				$(modalSelector + " #field-inspection-valvetype").val(
					dataAsset.valveType
				);
				$(modalSelector + " #field-inspection-tagno").val(
					dataAsset.tagNo
				);
				$(modalSelector + " #field-inspection-valvetagno").val(
					dataAsset.tagNo
				);
				$(modalSelector + " #field-inspection-manufacturer").val(
					dataAsset.manufacturer
				);
				$(modalSelector + " #field-inspection-equipmentname").val("");
				$(modalSelector + " #field-inspection-bodymaterial").val(
					dataAsset.bodyMaterial
				);
				...
				$(modalSelector + " #endconnection").val(
					dataAsset.endConnection
				);
				$(modalSelector + " #parentequipmentno").val(
					dataAsset.parentEquipmentNo
				);
				$(modalSelector + " #serialno").val(dataAsset.serialNo);
				$(modalSelector + " #parentequipmentdescription").val(
					dataAsset.parentEquipmentDescription
				);
				$(modalSelector + " #usagetype").val("");
				$(modalSelector + " #installationdate").val(
					dataAsset.installationDate
				);
				...
				// fill history data
				if (type === "maintenance") {
					$(modalSelector + " #field-maintenance-date").val(
						data.maintenanceDate
					);
					$(modalSelector + " #field-maintenance-repaired").val(
						data.isValveRepairedID
					);
					$(modalSelector + " #field-maintenance-description").val(
						data.maintenanceDescription
					);
				} else {
					$(modalSelector + " #field-inspection-date").val(
						data.inspectionDate
					);
					$(
						modalSelector + " #field-inspection-inspection-method"
					).val(data.inspectionMethodID);
					$(
						modalSelector +
							" #field-inspection-current-condition-limit-state-a"
					).val(data.currentConditionLeakeageToAtmosphereID);
					...
					$(modalSelector + " #field-inspection-effectiveness").val(
						data.inspectionEffectivenessID
					);
					$(
						modalSelector + " #field-inspection-function-condition"
					).val(data.functionCondition);
					$(modalSelector + " #field-inspection-description").val(
						data.inspectionDescription
					);
					$(
						modalSelector +
							" #field-inspection-test-pressure-if-any"
					).val(data.testPressureIfAny);
				}
